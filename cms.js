...
(()=>{
  ...
  function setupStoreCatalogue(records,store){
    ...
    const options=(select,values,label)=>{select.innerHTML=`<option value="all">All ${label}</option>`+values.map(value=>`<option value="${attrCart(value.toLowerCase())}">${esc(value)}</option>`).join('')};
    ...
    const setView=value=>{view=value==='list'?'list':'grid';store.dataset.catalogueView=view;document.querySelectorAll('[data-store-view]').forEach(button=>button.classList.toggle('active',button.dataset.storeView===view));try{localStorage.setItem('oss-store-view',view)}catch(error){}};
    const apply=()=>{
    ...
    toggle?.addEventListener('click',()=>openFilters(!panel.classList.contains('open')));document.querySelectorAll('[data-filter-close]').forEach(button=>button.addEventListener('click',()=>openFilters(false)));setView(view);apply();
  }
