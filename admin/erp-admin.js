...
loadTab=async function(tab){
  if(!['vessels','heavy-equipment','store'].includes(tab))return baseLoadTab(tab);
  ...
  try{return tab==='store'?await loadStorePublishing():await loadErpPublishing(tab)}catch(error){panel.innerHTML=errorState(error.message);toast(error.message,'error')}
};
...
async function storeSources(){
  ...
  return [...(vessels.data||[]).map(row=>({...row,source_table:'vessels',source_id:String(row.id),source_kind:'vessel',source_type:row.vessel_type||'Vessel'})),...(assets.data||[]).map(row=>({...row,source_table:'assets',source_id:String(row.id),source_kind:'equipment',source_type:row.category||row.vessel_type||'Equipment'})),...(inventory.data||[]).map(row=>({...row,name:row.name||row.item_name||row.description||row.sku||'Inventory item',source_table:'inventory_items',source_id:String(row.id),source_kind:'inventory',source_type:row.category||row.item_type||'Marine spare',stock_quantity:row.quantity_on_hand??row.quantity??row.stock_quantity??null}))];
}
...
function openStoreEditor(source){
  const existing=state.store.listingByKey.get(storeKey(source)),row=existing||{title:source.name,category:source.source_type,summary:source.description||'',image_url:'',condition:'Used',location:source.location||'Abu Dhabi, UAE',price_amount:'',currency:'AED',price_label:'Price on request',stock_quantity:source.stock_quantity??'',featured:false,published:true,sort_order:0};
  openDrawer(existing?`Edit ${source.name}`:`Offer ${source.name}`,'Public store listing',`<div class="erp-source-summary"><span class="eyebrow">Read-only ERP source</span><strong>${esc(source.name)}</strong><small>${esc(titleCase(source.source_kind))} · ${esc(source.source_type)}</small></div><form class="editor-form" id="store-publication-form"><label class="field">Listing title<input name="title" value="${attr(row.title)}" required></label><label class="field">Category<input name="category" value="${attr(row.category||source.source_type)}"></label><label class="field full">Public description<textarea name="summary" required>${esc(row.summary||'')}</textarea></label>${imageField('image_url','Product image',row.image_url||'')}<label class="field">Condition<select name="condition"><option ${row.condition==='New'?'selected':''}>New</option><option ${row.condition==='Used'?'selected':''}>Used</option><option ${row.condition==='Refurbished'?'selected':''}>Refurbished</option><option ${row.condition==='For scrap'?'selected':''}>For scrap</option></select></label><label class="field">Location<input name="location" value="${attr(row.location||'Abu Dhabi, UAE')}"></label><label class="field">Price amount<input name="price_amount" type="number" min="0" step="0.01" value="${attr(row.price_amount??'')}"></label><label class="field">Currency<select name="currency">${['AED','USD','EUR'].map(value=>`<option ${row.currency===value?'selected':''}>${value}</option>`).join('')}</select></label><label class="field">Price display<input name="price_label" value="${attr(row.price_label||'Price on request')}" placeholder="Price on request"></label><label class="field">Stock quantity<input name="stock_quantity" type="number" min="0" step="1" value="${attr(row.stock_quantity??'')}"></label><label class="field">Sort order<input name="sort_order" type="number" value="${attr(row.sort_order??0)}"></label><label class="publish-field"><input name="featured" type="checkbox" ${row.featured?'checked':''}> Feature on homepage</label><label class="publish-field"><input name="published" type="checkbox" ${row.published!==false?'checked':''}> Published in store</label><div class="editor-actions"><button class="primary-button" id="save-store-publication" type="submit">Save listing</button><button class="secondary-button" id="cancel-store-publication" type="button">Cancel</button></div></form>`);
...
async function saveStoreListing(event,source,existing){
  ...
  try{const price=String(data.get('price_amount')||'').trim(),stock=String(data.get('stock_quantity')||'').trim(),payload={source_id:source.source_id,source_table:source.source_table,item_type:source.source_kind,title:String(data.get('title')||source.name).trim(),category:String(data.get('category')||source.source_type).trim(),summary:String(data.get('summary')||'').trim(),image_url:await resolveImage(form,'image_url','store'),condition:String(data.get('condition')||'Used'),location:String(data.get('location')||'').trim(),price_amount:price===''?null:Number(price),currency:String(data.get('currency')||'AED'),price_label:String(data.get('price_label')||'Price on request').trim(),stock_quantity:stock===''?null:Number(stock),featured:form.elements.featured.checked,published:form.elements.published.checked,sort_order:Number(data.get('sort_order')||0),updated_at:new Date().toISOString()};
