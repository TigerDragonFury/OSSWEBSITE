...
function openStoreEditor(source){
  const existing=state.store.listingByKey.get(storeKey(source)),row=existing||{title:source.name,sku:source.sku||source.asset_tag||'',category:source.source_type,summary:source.description||'',image_url:'',condition:'Used',location:source.location||'Abu Dhabi, UAE',price_amount:'',currency:'AED',price_label:'Price on request',stock_quantity:source.stock_quantity??'',max_order_quantity:10,purchasable:false,featured:false,published:true,sort_order:0};
